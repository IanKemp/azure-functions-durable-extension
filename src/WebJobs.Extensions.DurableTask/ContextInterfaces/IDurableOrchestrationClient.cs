...
namespace Microsoft.Azure.WebJobs.Extensions.DurableTask
{
    ...
    public interface IDurableOrchestrationClient
    {
        ...
        /// <param name="returnInternalServerErrorOnFailure">Optional parameter that configures the http response code returned.
        /// If <see langword="true"/>, the returned HTTP response code will be a 500 when the orchestrator is in a failed state; when <c>false</c> it will
        /// return 200. Defaults to <see langword="false"/>.</param>
        ...
        /// </para>
        /// </remarks>
        /// <exception cref="ArgumentException">The instance ID does not correspond to a valid orchestration instance.</exception>
        ...
        /// <returns>A task that completes when the status has been fetched.</returns>
        ...
        /// </summary>
        /// <param name="createdTimeFrom">If specified, return orchestration instances which were created after that DateTime.</param>
        /// <param name="createdTimeTo">If specified, return orchestration instances which were created before that DateTime.</param>
        /// <param name="runtimeStatus">If specified, return orchestration instances which matches the runtimeStatus.</param>
        /// <param name="cancellationToken">If specified, can be used to cancel the status query operation.</param>
        /// <returns>The orchestration status for all instances.</returns>
        [Obsolete]
        ...
        /// <returns>An instance of <see cref="PurgeHistoryResult"/>.</returns>
        ...
        /// <param name="runtimeStatus">List of runtime statuses for querying instances for purging. Only <see cref="PurgeHistoryResult.Completed"/>, <see cref="PurgeHistoryResult.Terminated"/>, or <see cref="PurgeHistoryResult.Failed"/> will be processed.</param>
        /// <returns>An instance of <see cref="PurgeHistoryResult"/>.</returns>
        ...
        /// <param name="cancellationToken">Token that can be used to cancel the status query operation.</param>
        /// <returns>Each page of orchestration status for all instances and a continuation token for the next page.</returns>
        [Obsolete]
        ...
        /// </summary>
        /// <param name="instanceId">Instance ID of a previously run orchestrator to restart.</param>
        /// <param name="restartWithNewInstanceId">Optional parameter that configures if restarting an orchestration will use a new instance ID,
        /// or if the previous ID will be reused. Defaults to <see langword="true"/>.</param>
        /// <returns>A task that completes when the orchestration is started. The task contains the instance ID of the started
